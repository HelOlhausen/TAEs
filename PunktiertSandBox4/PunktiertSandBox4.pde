...
float radio = 10;
float peso = 10;
float posibilidadExistencia = 0.5;
...
public void setup() {
  ...
  attrMD = new BAttraction(new Vec(width * .5f, height * .5f), 100, .5f);
  attrMI = new BAttraction(new Vec(width * .5f, height * .5f), 100, .5f);
...
public void draw() {
  ...
  if (random(0,1)>posibilidadExistencia){
    crearParticula((int) random(0,width), (int) random((-height)/2,0));
  };
  //Updeteo la fisica
  physics.update();
